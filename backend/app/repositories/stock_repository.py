...
def get_by_product_and_location(db: Session, product_id: int, location_id: int)
    return db.execute(select(Stock).where(Stock.product_id == product_id, Stock.location_id == location_id )).scalar_one_none()
